...
class MentorApiIntegrationTest {
    ...
    private static final String PATH =
            "/api/v1/me/courses/mentor-test-course/lessons/" + LESSON_ID + "/mentor/messages";
    ...
    @MockitoBean MentorUseCase mentor;
    ...
    void returnsConversationHistoryAsJson() throws Exception {
        when(mentor.history(USER_ID, "mentor-test-course", LESSON_ID))
                .thenReturn(
                        List.of(
                                message(
                                        UUID.randomUUID(),
    ...
    void streamsNamedEventsWithoutExposingProviderDetails() throws Exception {
        MentorMessageView user =
                message(UUID.randomUUID(), MentorMessageRole.USER, "Where should I start?");
        MentorMessageView assistant =
                message(UUID.randomUUID(), MentorMessageRole.ASSISTANT, "Start small.");
        doAnswer(
                        invocation -> {
