...
@Sql(
        scripts = "/organization-test-data.sql",
        executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
@Sql(
        scripts = "/organization-test-cleanup.sql",
...
class OrganizationFlowIntegrationTest {
    ...
    void createsReplaysListsAndProtectsRosterThroughRealAdapters() throws Exception {
                ...
                post("/api/v1/me/organizations")
                        ...
                        .with(
                                jwt().authorities(
                                        ...
                                        .jwt(token -> token.subject(OWNER_ID)));
        ...
        mockMvc.perform(
                        get("/api/v1/organizations/{organizationId}/members", organizationId)
                                .with(
                                        jwt().authorities(
                                                        new org.springframework.security.core
                                                                .authority.SimpleGrantedAuthority(
                                ...
                                .with(
                                        jwt().authorities(
                                                        new org.springframework.security.core
                                                                .authority.SimpleGrantedAuthority(
                                                                "ROLE_STUDENT"))
                                                .jwt(token -> token.subject(OUTSIDER_ID))))
