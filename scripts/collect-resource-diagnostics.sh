...
[[ "$workload_kind" == 'catalog' \
  ...
  || "$workload_kind" == 'media-bandwidth' ]] \
  || fail 'DIAGNOSTICS_WORKLOAD_KIND must be catalog, authenticated-learning, learning-event, notification-websocket, ai-mentor, or media-bandwidth'
...
if [[ "$workload_kind" == 'media-bandwidth' ]]; then
  minio_container="$(required_value DIAGNOSTICS_MINIO_CONTAINER)"
fi

umask 077
printf 'epoch\tapp_cpu\tapp_memory\tapp_pids\tpostgres_cpu\tpostgres_memory\tredis_cpu\tredis_memory\tjvm_used_bytes\thikari_active\thikari_pending\tkafka_cpu\tkafka_memory\tkafka_pids\toutbox_pending\toutbox_oldest_age\tactive_websocket_sessions\tprovider_cpu\tprovider_memory\tprovider_pids\tminio_cpu\tminio_memory\tminio_pids\n' \
  >"$samples_file"
...
while [[ ! -e "$stop_file" || "$captured_samples" -lt "$minimum_samples" ]]; do
  ...
  [[ -z "$minio_container" ]] || stats_containers+=("$minio_container")
  ...
  if [[ "$workload_kind" == 'media-bandwidth' ]]; then
    minio_stats="$(container_stats_from_snapshot "$stats_snapshot" "$minio_container")"
    IFS=$'\t' read -r minio_cpu minio_memory minio_pids <<<"$minio_stats"
  fi
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    ...
    "$minio_cpu" "$minio_memory" "$minio_pids" \
    >>"$samples_file"
...
read -r sample_count max_app_cpu max_app_memory max_app_pids \
  ...
  max_provider_cpu max_provider_memory max_provider_pids \
  max_minio_cpu max_minio_memory max_minio_pids observed_span <<<"$(
    awk -F '\t' '
      NR == 1 { next }
      {
      ...
      END {
        printf "%d %.6f %.6f %d %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %d %.6f %.6f %.6f %.6f %.6f %d %.6f %.6f %d %d", count,
          ...
          maximum[18], maximum[19], maximum[20], maximum[21], maximum[22], maximum[23],
...
elif [[ "$workload_kind" == 'media-bandwidth' ]]; then
  summary_format='ai-learning-media-bandwidth-resource-v1'
  workload_json='{"identities": 40, "ratePerSecond": 8, "durationSeconds": 30, "courseSlug": "media-bandwidth-performance", "objectSizeBytes": 33554432, "rangeSizeBytes": 1048576}'
else
...
if [[ "$workload_kind" == 'media-bandwidth' ]]; then
  printf '  "minio": {"maxCpuPercent": %s, "maxMemoryPercent": %s, "maxPids": %s}\n' \
    "$max_minio_cpu" "$max_minio_memory" "$max_minio_pids" >>"$output_file"
fi
printf '}\n' >>"$output_file"
