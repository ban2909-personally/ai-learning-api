...
@Sql(
        scripts = "/notification-test-data.sql",
        executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
@Sql(
        scripts = "/notification-test-cleanup.sql",
...
class NotificationApiIntegrationTest {
    ...
    @MockitoBean private NotificationRealtimeDelivery realtime;
                                ...
                                .with(
                                        jwt().authorities(
                                                        new org.springframework.security.core
                                                                ...
                                                                "ROLE_STUDENT"))
                                                .jwt(token -> token.subject(FIRST_USER))))
    ...
    void marksAnOwnedNotificationReadButHidesAnotherUsersRow() throws Exception {
        mockMvc.perform(
                        patch("/api/v1/me/notifications/{notificationId}/read", NEWEST)
                                .with(
                                        jwt().authorities(
                                                        new org.springframework.security.core
                                                                .authority.SimpleGrantedAuthority(
                                                                "ROLE_STUDENT"))
                                                .jwt(token -> token.subject(SECOND_USER))))
