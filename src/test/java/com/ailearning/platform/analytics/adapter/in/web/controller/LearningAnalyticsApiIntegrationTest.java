...
class LearningAnalyticsApiIntegrationTest {
    ...
    void requiresAuthentication() throws Exception {
        mockMvc.perform(get("/api/v1/me/learning-analytics")).andExpect(status().isUnauthorized());
    }

    @Test
    void returnsOnlyAuthenticatedUsersBoundedCompletionSummary() throws Exception {
        mockMvc.perform(
                        get("/api/v1/me/learning-analytics")
                                .param("courseLimit", "1")
                                .with(
                                        jwt().authorities(
                ...
                .andExpect(
                        jsonPath("$.courses[0].courseId")
                                .value("8aff449f-cfa6-4ed8-a3e7-5461090ee101"))
